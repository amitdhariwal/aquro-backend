...
router.put('/payments/:paymentId', async (req, res) => {
  try {
    const updated = await SupplierPayment.findByIdAndUpdate(req.params.paymentId, req.body, { new: true });
    res.json(updated);
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
...
router.delete('/payments/:paymentId', async (req, res) => {
  try {
    await SupplierPayment.findByIdAndDelete(req.params.paymentId);
