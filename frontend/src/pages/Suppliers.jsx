...
export default function Suppliers() {
  ...
  const openSupplierLedger = async (supplier) => {
    ...
    try {
      const resInv = await fetch('http://localhost:5000/api/inventory/history');
      if (resInv.ok) {
  ...
  const fetchSupplierPayments = async (supplierId) => {
    try {
      const resPay = await fetch(`http://localhost:5000/api/suppliers/${supplierId}/payments`);
      if (resPay.ok) {
  ...
  const handleAddPayment = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/suppliers/${selectedSupplier.id}/payments`, {
        method: 'POST',
  ...
  const handleSave = async (e) => {
    ...
    try {
      const url = isEdit 
        ? `http://localhost:5000/api/suppliers/${formData.id}` 
