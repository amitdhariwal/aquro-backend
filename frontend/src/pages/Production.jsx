...
export default function Production() {
  ...
  const fetchProductions = async () => {
    try {
      ...
      const resHist = await fetch('http://localhost:5000/api/production/history');
      if (resHist.ok) {
  ...
  const addHistoryLog = async (type, batch, details, payload = null) => {
    try {
      ...
      await fetch('http://localhost:5000/api/production/history', {
        method: 'POST',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        ...
        await fetch(`http://localhost:5000/api/production/${formData.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (invRes.ok) {
          ...
          for (let item of inventory) {
            ...
            if (shouldUpdate) {
              await fetch(`http://localhost:5000/api/inventory/${item._id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id, batch) => {
    if (window.confirm(`Are you sure you want to delete batch ${batch}?`)) {
      try {
        ...
        await fetch(`http://localhost:5000/api/production/${id}`, { method: 'DELETE' });
